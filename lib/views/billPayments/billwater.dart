...
class BillWaterScreen extends StatefulWidget {
  @override
  _BillWaterScreen createState() => _BillWaterScreen();
}

class _BillWaterScreen extends State<BillWaterScreen> {
        ...
        title: Row(
          children: <Widget>[
            ...
            Text(
              "Water Bill",
              style: TextStyle(
                  fontWeight: FontWeight.bold, fontFamily: 'CalibriBold',
                  fontSize: 20)
            ),
            Spacer(),
            Icon(
                          ...
                          onPressed: () {
                            if (_selectedCompany == "-select account-") {
                              ...
                              BillPayment billpayment = BillPayment(
                                  ...
                                  comments: narration,
                                  utilityname: "WATER",
                                  utilitycompany: _selectedProvider,
                                  MyAccountNumber: _selectedCompany,
                                  AccountNumber: _selectedCompany);

                              if (_selectedProvider == "Blantyre Water Board") {
                                isNitel = true;
                              ...
                              if (isFdh) {
                                if (_selectedProvider ==
                                    'Lilongwe Water Board Prepaid Payment') {
                                  ProviderCode = "LWBPrepaid.Transaction";

                                  paidTransactionData = PaidTransactionData(
                                      amount: amount,
                                      clientTransactionIdentifier:
                                          randomNumber.toString(),
                                      meterNumber: billnumber);

                                  utilityTransaction = UtilityTransaction(
                                      accountNumber: _selectedCompany,
                                      username: username,
                                      sessionId: randomNumber.toString(),
                                      billPaymentApplicationName: ProviderCode,
                                      comments: narration,
                                      lwbPrepaidTransactionData:
                                          paidTransactionData);
                                } else if (_selectedProvider ==
                                    'Lilongwe Water Board Postpaid Payment') {
                                  ProviderCode = "LWB.Postpaid.Transaction";
                                  paidTransactionData = PaidTransactionData(
                                      amount: amount,
                                      clientTransactionIdentifier:
                                          randomNumber.toString(),
                                  ...
                                  utilityTransaction = UtilityTransaction(
                                      ...
                                      billPaymentApplicationName: ProviderCode,
                                      comments: narration,
                                      lwbPostpaidTransactionData:
                                          paidTransactionData);
                                } else if (_selectedProvider ==
                                    'Southern Region Water Board Postpaid Payment') {
                                  ProviderCode = "SRWB.Postpaid.Transaction";
                                  ...
                                  utilityTransaction = UtilityTransaction(
                                      ...
                                      billPaymentApplicationName: ProviderCode,
                                      comments: narration,
                                      srwbPostpaidTransactionData:
                                          paidTransactionData);
                                } else if (_selectedProvider ==
                                    'Blantyre Water Board Postpaid Payment') {
                                  ProviderCode = "BWB.Postpaid.Transaction";
                                  paidTransactionData = PaidTransactionData(
                                      amount: amount,
                                      clientTransactionIdentifier:
                                          randomNumber.toString(),
                                  ...
                                  utilityTransaction = UtilityTransaction(
                                      ...
                                      billPaymentApplicationName: ProviderCode,
                                      comments: narration,
                                      bwbPostpaidTransactionData:
                                          paidTransactionData);
                                } else if (_selectedProvider ==
                                    'Blantyre Water Board Prepaid Payment') {
                                  ProviderCode = "BWB.Prepaid.Transaction";
                                  paidTransactionData = PaidTransactionData(
                                      amount: amount,
                                      clientTransactionIdentifier:
                                          randomNumber.toString(),
                                  ...
                                  utilityTransaction = UtilityTransaction(
                                      ...
                                      billPaymentApplicationName: ProviderCode,
                                      comments: narration,
                                      bwbPrepaidTransactionData:
                                          paidTransactionData);
                                }

                                if (canclick) {
                                  print("Provider Code" + ProviderCode);
  ...
  Future<void> getAccounts() async {
    ...
    Member member =
        Member(username: username, password: P, OPERATION: "Accounts");

    try {
      fetchAccount(member, token: AuthToken).then(
          (value) => {polulate(value.ministatement, value.Accounts, value)});
    } on Exception catch (_) {
      print(_);
      Fluttertoast.showToast(
  ...
  Future<void> getProviders() async {
    ...
    try {
      newfetchProviders("WATER", member, token: AuthToken).then((value) => {
            polulateProviders(
                value.ministatement, value.utilityProviders, value)
          });
    } on Exception catch (_) {
      print(_);
      Fluttertoast.showToast(
  ...
  Future<void> pay(Member member) async {
    ...
    try {
      payBill(member, token: AuthToken).then((value) => {payresponse(value)});
    } on Exception catch (_) {
      setState(() {
        canclick = true;
      });
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: SingleChildScrollView(
              child: ListBody(
                children: <Widget>[
                  Text("Source Account: " + _selectedCompany,
                      style: Theme.of(context).textTheme.bodyText1),
                  Text("Utility Provider: " + _selectedProvider,
                      style: Theme.of(context).textTheme.bodyText1),
              ...
              TextButton(
                ...
                onPressed: () {
                  try {
                    ...
                    payUtilityBill(utilityTransaction, token: AuthToken)
                        .then((value) => {
                          payresponse(value)
                        });
                    Navigator.of(context).pop();
  ...
  void polulate(
      String name, List<String> accounts, AccountResponse accountResponse) {
        ...
        if (newutilityprovidersResponse.utilityProviders != null) {
          setState(() {
            ...
            for (int i = 0; i < Providers.length; i++) {
              ...
              billerList.add(providerName[i]['UTILITYBILLERID']);
